...
class ServiceManager extends Loggable {
	...
	def onVisibilityChange(): Unit = {
		...
		if (changed && active) {
			logw("APP", s"${contextName()}   >   APP  VISIBLE= $lastVisible")
...
private[app] object Dependencies extends collection.mutable.HashSet[(AppService, AppService)] {
	...
	def withParents(like: AppService => Boolean)(code: AppService => Unit)(implicit child: AppService) = collect { case (p, c) if  c == child && like(p) => code(p) }
